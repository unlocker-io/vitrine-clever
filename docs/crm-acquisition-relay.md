...
Le navigateur place un UUID v4 opaque dans son champ caché de tentative. Il est aussi conservé en stockage first-party, sous une clé versionnée limitée au formulaire configuré, avec le même TTL logique que le mapping serveur : rechargement, nouvelle visite et `reset` d’un corps identique portent donc la même clé C1b et ne recréent ni demande, ni lead, ni tâche. Le stockage contient uniquement `{v,id,expires_at}` ; il n’inclut aucune donnée de prospect, URL, cookie publicitaire ou consentement. `localStorage` ne supprime pas automatiquement une valeur à son échéance : le bootstrap la considère expirée et la supprime au prochain chargement où le relais est encore actif pour ce formulaire. Si le relais est désactivé, si l’ID du formulaire change ou si le navigateur ne revient pas, la valeur opaque peut rester physiquement présente au-delà de cette date ; après purge du mapping serveur elle n’est plus raccordable à une demande CRM. L’échéance repose sur l’horloge du navigateur : une correction importante de cette horloge peut avancer ou retarder la rotation locale. Ces limites font partie de la validation juridique préalable à `CRM_ACQUISITION_CONSENT_GATE_CONFIRMED=1`.

Le bootstrap est un asset same-origin enregistré par WordPress, sans JavaScript inline : MetForm pouvant remplir son wrapper après le chargement de page, il observe ce rendu asynchrone pendant dix secondes au plus, se débranche dès que le champ est trouvé et reste fermé si Web Crypto ou le stockage first-party ne sont pas disponibles. Un listener `reset` ciblé sur ce seul formulaire réinsère la même tentative stable. Le serveur la résout vers un UUID d'idempotence CRM différent, persisté avec ses dates de création et d'expiration dans une option WordPress non autoloadée sous un nom hashé. L’insertion est un `INSERT IGNORE` MySQL/MariaDB non-écrasant, et non `add_option()` (qui UPSERT dans WordPress 6.9) : deux doubles-soumissions concurrentes portent donc la même clé C1b. Une réponse C1b 409 ou 422 fait tourner atomiquement l'UUID CRM associé sans tourner l’UUID navigateur : un corps modifié peut ainsi être renvoyé avec la nouvelle clé serveur après correction, sans conflit permanent.
...
Le harnais PHP du relais fait partie de `composer test` via `@relay-tests` et ne suppose pas la présence implicite de Node. Le harnais DOM utilise le runtime déclaré dans `package.json` (`Node >=20 <25`) et s’exécute séparément avec `npm run test:relay-dom`. Le gate agrégé attendu avant PR est donc `composer test && npm run test:relay-dom`. Ces harnais restent autonomes et ne contactent ni WordPress réel, ni CRM, ni compte tiers. Le dépôt ne contient actuellement aucun workflow CI : tant qu’un runner n’automatise pas ces deux commandes, leurs résultats doivent être consignés manuellement dans la PR.
